...
A severe downside of the URL preview is that the end of the preview is cut in case the URL is too long.
 ...
 Ultimately, the user will trust this URL even he should not.
	\item \textit{Users Like Clicking} Clicking on links is practical and convenient. Due to this users like clicking on links. A study conducted by Karlof et al.~\cite{karlof2009conditioned} revealed that humans tend to develop automatic responses to recurring situations, so called click whirr responses.
Such click whirr responses include, for instance, filling out forms or following links in e-mails from trusted senders.
Breaking bad and steady habits is generally a difficult task.
However, the habit of carelessly filling out forms involves more actions compared to simply clicking on a link.
...
Ultimately, we do not believe that we can hinder users from clicking on links, but we hope that we can make them re-think their decision of providing their sensitive data to a potentially malicious website.
\end{enumerate}
