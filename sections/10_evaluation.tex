...
In \autoref{s:target_group_def} we defined preconditions users have to hold in order to match our target group.
...
\begin{description}
	\item[Attackability:] Generally, we took care that the people we tried to recruit do not have extensive prior knowledge on this topic.
...
This way we assured that the considered participants were potential targets of phishing.
	\item[Android Users:] Our target user should have an Android smartphone as only those can make use of our app on a regular basis.
Since we provided the required smartphones to the users during the study we had no need to recruit participants who obligatorily own an Android smartphone.
For the study we considered it sufficient to be a smartphone owner in general.
...
For the user study this was not possible.
Therefore, we created a motivation by raffling an Amazon gift certificate so that users had an incentive to attend our study.
\end{description}


Finally, we aspired to recruit participants who are not close friends of ours.
This is not a specific precondition we set for our target group, but we consider it relevant in order to minimize biases es far as possible
