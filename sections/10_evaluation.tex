...
\begin{enumerate}
	\item \textbf{Informed Consent} Before starting the user study the participants have to sign an informed consent.
...
Optionally, the user can agree with the anonymous publication of the:
	...
	\item \textbf{General-Survey Before} At the beginning the participants have to fill out a general survey, where they have to judge their own knowledge on the topic of Internet security in general.
 ...
 Moreover, it contains 8 further website screenshots of which 4 have phishing and the remaining 4 have valid URLs.
	...
	\item \textbf{General-Survey After} Here, the participants are asked to fill out a form with questions to their demographics.
 This form does also contain questions related to the SUS and some other questions regarding their impression of the app.
	
	\item \textbf{Certificates} At this point the official study is finished.
...
Next, the gift certificate is raffled.
	...
	\item \textbf{Debrief} Finally, an optional debrief follows, where the users can ask questions or provide their remarks in person.
...
The exact procedure of our measurement is described in \autoref{s:measurement}.


\begin{enumerate}
	\item \textbf{Hypothesis 1 - Mistakes} After playing the app, the users make significantly less mistakes when deciding whether a website is a phish or not.
	\item Measurement: Correct answers in ``Website-Survery After'' $>>$ correct answers in ``Website-Survery Before''
	...
	\item Measurement: Number of URL markings in ``Website-Survery After'' $>>$ number of URL markings in ``Website-Survery Before''
	\item \textbf{Hypothesis 3 - URL Comprehension} After playing the app the user understands the importance of the domain of a URL as the only criteria to detect phishing websites.
	\item Measurement: Number of marked URL domains in ``Website-Survery After''  $>>$ number of marked URL domains in ``Website-Survery Before'' 
	\item \textbf{Hypothesis 4 - Good Usability} The app usability is above average.
	\item Measurement: According to~\cite{sus}  a System Usability Scale (SUS) $>$ 68 can be considered above average usability.
\end{enumerate}
