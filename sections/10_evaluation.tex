...
\end{description}
In \autoref{s:final_study_materials} copies of our e-mails to the professors and of our flyer can be consulted.
...
Additionally, the participant who performed best was awarded with a ``Golden Anti-Phish Certificate'', all other participants received a ``Silver Anti-Phish Certificate'' (cf. \autoref{s:final_study_materials}).
...
The survey can be consulted in \autoref{s:final_study_materials}.
 ...
 The list of the used URLs can be found in \autoref{s:final_study_materials}.
Note that we used the same order for every participant's survey, i.e. everybody was confronted with these URLs in the same order.
 ...
 To save time, we skipped the introduction part~2  (how to access the address bar) for the user study.
 The user has half an hour to play.
 Afterwards, they are asked to put the smartphones aside.
 Then, we collect the smartphones and note the achieved points for each level.
...
An example corresponding survey sheet can be found in the appendix \autoref{s:url_survey}.
...
In this case one can see that the subdomain is inside the circle. We coded this sample as URL accordingly.
...
We start with discussing the representativeness of our participants and proceed with the evaluation of our hypotheses. 
Subsequently, we continue with further exploration of our results, followed by discussing the limitations of our study.
...
\begin{description}[leftmargin=0cm]
	\item[Attackability:] Generally, we took care that the people we recruited do not have extensive prior knowledge on the topic of phishing.
...
The problem with the phishing survey was that it did not give us enough indications whether a particular participant was too familiar with the topic.
...
For the user study our flyer was supposed to draw interested persons to participate.
...
\subfigure[After]{\label{fig:domain_after}\includegraphics[width=0.45\textwidth]{hyp3a.pdf}}
\caption{Domain marked}
...
\begin{description}[leftmargin=0cm]
\item[Hypothesis 1:]
\autoref{fig:hyp1results} shows the results of our study according to hypothesis 1. One can clearly see that the majority of the users identified more URLs correctly after using the app than before. While most participants correctly identified 8 out of 16 (50\%) websites before they played the app, almost everyone gave correct answers to at least 22 out of 24 websites afterwards (cf. \autoref{fig:hyp1resultsbefore} and \autoref{fig:hyp1resultsaall}).
One could argue that this increase is based on the fact that the examples are mainly the same in the website-survey after, i.e. the reason for the users' improved performance is based on learning effects. \autoref{fig:hyp1resultsanew} however shows that the users also performed well for the new URLs. Therefore, we assume the learning effects are negligible.
...
Our null hypothesis is $H_{0}: x_{1} >= x_{2}$ and the alternative hypothesis is $H_{1}: x_{1} < x_{2}$, where x$_{1}$ represents the number of URLs which were correctly answered before playing the app and x$_{2}$ represents the number of URLs which were correctly identified after playing the app.
...
Interesting is that only 3 users (17.65\%) \textit{always} marked the URL before playing the app.
...
In the website-before survey we were not able to clearly ask the users to mark the domain when it was the base of their decision because we would have then pointed them towards looking at the URL or even at the domain.
This would have influenced the results of hypothesis~2.
Since we could not formulate this question clearly, a user might have marked the whole URL even if his decision was based only on a part of it (for example, the domain).
Consequently, we were not able to clearly identify what the users' main source of decision was in the website-survey before.
We were aware of this problem beforehand but saw no other option than formulating the question in such an open form.
After playing the app the user knew that they were expected to mark the domain.
...
\item[Hypothesis 4:]
According to the answers that the users gave in the  SUS section of the general-survey after (\autoref{s:final_study_materials}) we calculated a SUS score of 83.1. This is considerably higher than 68 which means that we can consider our app usable.
\end{description}
...
\begin{description}[leftmargin=0cm]
	...
	However, a reasoning does not always have to rely on the domain itself. 
	...
	Also, there were plenty of users who detected a phish and marked the attacked part instead of the domain (cf. \autoref{fig:m_attack_recognized}).
	...
	\item[False Negatives and Positives:] Additionally, we separately considered whether the user falsely accepted fraudulent or rejected legitimate websites. Our outcomes show that before in average the user rejected 4.9 legitimate websites and accepted 2.1 phishing websites.
	...
	In future this should be tested in a more realistic scenario where the users' attention is less increased towards looking for phishing websites and where the users are not in lab situations.
	\item[User Opinions to App:] A part of our surveys aspired to understand the users' opinions to our app.
	Section 3 of our general-survey after (cf.~\autoref{s:final_study_materials}) contains statements which the users had to assess with the aid of a 5 point Likert scale.
	...
	All other statements of the questionnaire were strongly agreed with in median (median of 5).
	The text legibility of our app texts received a median score of 5.
	Our study participants strongly agreed (median of 5) that our app helped them identify phishing websites in the future.
	...
	We assume that some users started to roughly skim our texts for relevant information (for example, by mainly looking at the examples) after they understood the importance of the domain.
...
\end{enumerate}
	...
	This aspect should definitely be considered in future work.
	\item[User Self-Assessment:] Contrary to our expectations, users assessed themselves rather correctly. When asked for their ability of distinguishing legitimate from fraudulent websites at the beginning of the study they rated themselves with a median of 3 on a 5 point Likert scale.
Their actual performance in the website-survey before was likewise (56\% correctly identified URLs). After using the app they were asked whether the app helped them identify phishing websites. They rated 5 out of 5 and their performance was accordingly good (87\% correctly identified URLs).
...
One idea to approach this challenge might be to provide a ``I don't know'' button in addition to the check mark (no phish) and cross mark (phish) buttons. 
...
4 participants stated that they found the explanations of the app very good and easy to understand.
...
We believe that the number of examples are reasonable, assumed that a user does not finish the app at a stroke.
...
\begin{description}[leftmargin=0cm]
	...
	Therefore, they likely behaved differently during our study~\cite{wu2006security, egelman2007security}..
	...
	Additionally, for the website-survey before and after they were explicitly asked to indicate whether the websites were phishing websites or not.
	That is to say, the user automatically increased his attention towards answering these kinds of questions.
	Designing an in situ study, where the participants would have been in their usual environment and would have not known about their participation, was not considered because such a design is ethically and legally questionable. 
...
\end{description}
Despite the limitations of our study we are confident that we obtained a good insight into the effectiveness of our app.
...
Even though there might be some learning effects we are confident that this increase is not mainly attributable to that.
...
Yet, it was striking that after playing the app the majority of the participants, in total 14, \textit{always} based their decision on the URL compared to before playing the app, where only 3 users did.
The third hypothesis stated that the users understand the structure of URLs better after playing our app.
As discussed above, we had concerns about the design of the question that was intended to test this hypothesis.
...
Nevertheless, the results show that after playing the app there was a shift in the users' focus towards marking the domain.
Our last hypothesis stated that our app is usable above average.
With a SUS score of 83.1, which is considerably higher than 68, we can consider our app usable.

In addition to testing the hypotheses we received generally positive feedback from the users.
Most of them had the feeling they learned something from our app.
...
Yet, there are still some improvements that should be considered when someone develops for a next version of the app.
First of all, we think that the part talking about HTTP and HTTPS should be introduced earlier.
Some users mainly marked the scheme or the padlock in the website-survey before and fell for phishing websites making use of HTTPS.
...
Furthermore, it might be reasonable to modify the app behavior depending on the user skills and performance.
...
Another example is that the app could skip the proof part (identify domain after detecting a phish) in case there is an indication that the user understood how to identify the domain.
A simple form of that is already implemented where the proof is shown up to a predefined level.
We think it is more reasonable to base the skipping and other possible extensions on the user performance. 

In conclusion, our evaluation shows that our app is beneficial to educate users on phishing. More precisely, the app helped most of the participants make improved decisions regarding the legitimacy of  URLs. The users appreciated the app and also provided valuable comments for possible improvements. All in all, we can say that we achieved the main goal of our app.
