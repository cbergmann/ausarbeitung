...
XSS vulnerabilities result from a web application's usage of content from external sources, such as search terms, auctions or user reviews of a product.
 This type of data supply can be misused and instead of delivering the expected kind of data malicious scripts can be injected.
	...
	With the aid of common Search engine optimization techniques the phisher tries to rank his phishing page above the legitimate webpage. By doing this he might trick users that do not know the valid URL of the legitimate webpage into visiting his phishing page.
...
Due to the drawbacks of embedded learning (legal issues) and comparison based (unrealistic) approaches we believe that a mixture of the game and quiz based approach containing some text is the best way to go in order to create an incentive for the users. 
